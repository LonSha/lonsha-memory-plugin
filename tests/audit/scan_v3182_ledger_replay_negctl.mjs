...
const CASES = [
    ...
    ['N3-报告落点字段改名', IDX,
        ...
        'all', 1,
        '留痕字段被改名，R3 必须翻红（回放发生了但没人看得见）；此处用 all，因为新增留痕点不应让本组作废'],
    ['N4-登记表 id 重复', LR,
        "id: 'diary', label: '日记'",
        "id: 'money', label: '日记'",
        1, 1,
...
function runCase(c) {
    ...
    try {
        ...
        if (file) {
            ...
            // expectHits 可为数字（恰中）或 'all'（至少 1 次，全量替换）。
            //   'all' 用于「字段整体改名」类破坏：该类断言不依赖出现次数，
            //   而每新增一处留痕点都会让硬写的数字失效（组作废而非真红）。
            const hitsOk = (expectHits === 'all') ? (hits >= 1) : (hits === expectHits);
            if (!hitsOk) {
