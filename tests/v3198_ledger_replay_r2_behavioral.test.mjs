...
test('v3198【0】版本 / manifest / package ≥ 3.198.0', () => {
  ...
  assert.ok(vnum(v) >= vnum('3.201.0'), `index.js 版本 ${v} < 3.198.0`);
  assert.ok(vnum(manifest.version) >= vnum('3.201.0'), `manifest ${manifest.version} < 3.198.0`);
  assert.ok(vnum(pkg.version) >= vnum('3.201.0'), `package ${pkg.version} < 3.198.0`);
});
