...
4. **v3.147.0（成熟度横评补强落地）**：
   - **shujuku 启示：API 凭据 401/403 冷却机制 (Credential Cooldown)**：`fetchWithTimeoutRetry` 捕获 401/403 后自动启动 30 分钟冷却，跨任务拦截重复请求，彻底消除 401/403 错误引发的网络风暴与控制台刷屏；保存配置（`saveConfig()`）或调用 `clearApiCooldowns()` 即时重置。
   - **baibai 启示：向量层双 hash 分层对账 (`docHash` vs `payloadHash`) & `embedCache`**：区分文本内容 hash（`docHash`）与元数据/状态 hash（`payloadHash`）。`VectorStore` 维护 `embedCache`，`addVector`/`addVectorAuto`/`import` 自动以 `docHash` 预热，检索/重复添加已知文本时 API 调用降为 0 次。
