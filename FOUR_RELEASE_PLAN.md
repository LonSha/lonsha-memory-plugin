...
当前：LonSha 3.224.0（**O-2 优化批**：「没给」与「给了 0」在**场所面**上的判开此前是**假的** ——
R3-D 把 `num(x) ?? 0` 改成 `numOrNull(x) ?? 0`，而对 `` / `null` / `[]` / 非数字串两条路径结果完全相同：
**那是改名不是修复**；本版把 import 七格如实判成 `null`、写侧三处改成「取不到即拒绝」，
并收紧 `v3222` A1（**它此前把未修状态写成断言，即判据在保护缺陷**）+ 新增 A2b 覆盖写侧）/
RubyPhone 3.2.0（O-1 消费侧**无需抬版**：本轮未新增外供键；既有键的取值域变化
`atFloor` / `firstFloor` / `lastFloor` ⇒ `number | null` **已被下游既有读法覆盖** ——
`place-data.js` 用的正是 `numOrNull`、视图把 `null` 渲染成空、把 `0` 渲染成「第0楼」）。
...
- **优化方向（O 批）：已启动** —— O-1（场所面「没给」与「给了 0」的真判开）**已完成**（v3.223.0，见上）；
  O-2（同族普查：回放/前移层的同一根因第二次发病）**已完成**（v3.224.0，见上）；
  其余（O-3 判据灵敏度体检 / O-4 门禁隔离与耗时 / O-5 性能取证 / O-6 死代码方法级扫描）待实施。
