...
撤掉三处把这个例外白名单化的既有判据；外供面未见变化，改的是**回滚路径上的内部位移**）/
RubyPhone 3.2.0（R3-E 消费侧**无需抬版**：下游全仓对 `ledger-replay` / `replayShift` / `stmLtm` /
`短期长期` 零命中，其 `coverage()` 消费的是另一本账 `floor-ledger.js`）。
