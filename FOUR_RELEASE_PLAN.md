...
## Gate P-2：index.js 宿主巨兽分诊（**已完成（分诊）**，lonsha v3.231.0）
- Structural: Local Fix；Execution: Local Fix Only；授权：approved local fix（自主迭代授权内）。
- 主题：**先分诊再决定拆不拆**。`index.js` 16922 行（占全仓 46%）此前只有一句话的描述。
- 实测：成员 559 个；TOP5 成员 3668 行 = **21.7%**；TOP40 = **51.3%**；
  10 条前缀域规则只覆盖 **24.7%**；over-80 行成员 32 个；缝合模块 **34 个 / 45 个引用点**。
  形状＝**少数超大成员 + 长尾小成员**。
- **结论（按读数否掉「按域拆」）**：① 最大成员是**生命周期接线本身**（onMessageReceived 1217 行 /
  onBeforeGeneration 503 行 / registerEvents 368 行），**外部引用极低恰是「唯一入口」而非「松散」**；
  ② 引用读数只能当上界（`import` 4 处 / `constructor` 3 处同名混算）；③ 规则外成员内部互调 4696 处，
  且九道门禁**无一道**检查跨模块成员可见性（拆错＝运行期 this 丢失，只有实机看得见）。
  唯一有读数支持的轴是「**成员级预算 + 文件规模上界**」，本版只立判据不动手。
- 同轮两处「读数看起来也像读数」的教训：探针首版按 4 空格切分 ⇒ 只切出 4 个方法（把 `if (` 与
  `const X = …` 当成了方法签名）；缝合模块匹配首版「遇右括号即止」⇒ 假零（真实调用第一参是箭头函数）。
- 落地：`tests/audit/host_beast_probe.cjs` + `tests/audit/host_beast_baseline.json`（读数**零手抄**，
  脚本从探针输出直接落盘）+ `tests/v3232_host_beast_triage.test.mjs`（14 项，含三条**镜像仓**负控制）。
- 边界：① **未拆任何东西**（index.js 一行未动）；② 未做 AST 级归属（沿用本仓文本口径）；
  ③ 未覆盖运行期可见性；④ 未排拆分优先级。

## Gate F-3（上游侧）：同楼同刻 ≥2 在场读数（**已完成**，lonsha v3.232.0）
- Structural: Local Fix；Execution: Local Fix Only；授权：approved local fix（自主迭代授权内）。
- 主题：`presence` 记着「谁在哪一楼」，但**没有出口回答「同一层楼里有没有两个人」** —— 而那正是
  冲突的事实前提；下游只能自己遍历再分组（同一口径抄 N 份的种子）。
- **★ 边界**：**只给事实，不给判断** —— 返回面只有楼层 / 地点键 / 路径 / 名单 / 人数，
  **无任何「冲突激烈程度」字段**（账本能答「两人都在钟楼」，不能答「他们会不会打起来」）。
- 口径：「没给楼层」不进任何一层楼且如实计数；名单与地点键**同口径**（拼音序）稳定排序；
  有界（`MAX_CO_PRESENCE_ROWS=120`）且 **`count` 是截断前的真实条数**；`summary()` 带上该面。
- **探针当场抓到的真缺陷（三态塌成两态的新形态）**：`coPresence('没给')` 与 `coPresence()` 同义
  —— 「给了但解不出」被当成「没给」，读数从空变全量而调用方看不出差别（与 O-1 同族）；
  已改为返回空 + `rejected: 'floor-unparsable'`，而真给第 0 楼仍按第 0 楼算。
- 落地：`scene-book.js` 三处 + `tests/v3233_co_presence_and_notes.test.mjs`（15 项，含三条真源码破坏负控制）。

## Gate F-6（上游侧）：两条观察项的口径自述（**已完成**，lonsha v3.232.0）
- 主题：T17（`shiftFloorRefs` 重复调用会再次平移）与 T16（未收结局事件时如实 pending）
  两条观察项此前**只写在注释与文档里**；本版做成机器可读自述 `observationNotes()`，
  下游可在自己的诊断页如实转述（那才是「如实声明」真正生效的地方）。
- **不改口径本身**：T17 的重复平移仍会发生（模块内加去重需要「记住哪些楼层已平移过」，
  那份记忆会成为第二个真源）；T16 仍如实停在 pending（不猜结局）。两条都写明**调用方职责**。
- 本方法**纯读、无副作用**，由套件 A3 钉住（破坏成「自述时顺手改状态」必须转红）。
- 落地：同上（同一文件、同一套件）。

## 状态检查点

- **F-2（上游侧）：已完成**（lonsha v3.233.0）—— 事件段的 `source` 从 40 字自由文本折成
  **受控分级四态**（extract / platform / other / none，`other` 保留原串）；平台标签由登记方**显式给出**
  （T11：不作文本猜测归类）；构成面恒定九键、**计数恒为截断前真值**、**无任何判断字段**
  （7 个禁用词在序列化 JSON 上逐个钉住零出现）；快照新增 `eventPlatforms` 四态面，自检新增「事件来源」行。
  同轮探针抓到两条真缺陷：① 证据面事件账**出处列恒 null**（条目顶层本无 floor）；
  ② `copySegment` **非幂等**（`finite(null) === 0` ⇒ 二次归一化把「没给」塔成「第 0 楼」，与 O-1/O-2/T8 同族）。
  **反坐实**：计划行文把 `event-chain.js` 列为交付面 —— 实测它是 agent run 生命周期校验（`/platform/i` 零命中）。
  v3234（24 项，含六条真源码破坏负控制）。
- **F-3 / F-6（上游侧）：已完成**（lonsha v3.232.0）- **F-3 / F-6（上游侧）：已完成**（lonsha v3.232.0）—— 同楼同刻事实面（不含判断）+ 两条观察项
  的机器可读自述（不改口径）；探针抓出「给了但解不出 ≠ 没给」并修掉；v3233（15 项）。
- **P-2：已完成（分诊）**（lonsha v3.231.0）—— 宿主巨兽分诊：559 成员 / TOP5 21.7% / TOP40 51.3% /
  前缀域覆盖 24.7% / 缝合模块 34 个；「按域拆」按读数**否掉**（最大成员是生命周期接线、引用读数只能当上界、
  规则外互调 4696 处）；唯一有读数支持的轴（成员级预算 + 文件规模上界）留待后续版本，本版只量不拆；
  v3232（14 项，含三条镜像仓负控制）。
