...
**主题：P-1 死方法处置 —— O-6 冻结台账上的 4 条零引用方法逐条实测后删除定义，台账归零；同轮修掉
三处判据自身缺陷（含一处「判据在保护已被设计淘汰的状态」与一处「负控制因自扫而变假绿」）。**

### 处置依据（逐条实测，不靠推演）

| 方法 | 分类 | 处置依据 |
|---|---|---|
| `_legacyHybridMerge` | 遗留融合 | 自 v3.50 引入 RRF 后无调用者（v3.156 起由 `hybridMerge` 取代） |
| `breakPromise` | 承诺账本 | 产品路径统一走 `resolvePromise(id, status)`；连测试也没调 |
| `callGeneric` | 旧生成路径 | 面向旧 `/api/v1/generate`；产品路径走 `chat/completions` |
| `queryByFloor` | 退役测试 | 唯一消费者在 `tests/archived/`（不参与跑批） |

**一处反坐实（重要）**：O-6 台账曾记「删 `_legacyHybridMerge` 需连配置键一起处置」。
本版实测**该推测不成立** —— 配置键 `hybridAlpha` 的活消费点是 `hybridMerge()`（删除前 index.js:6832），
与遗留方法**无绑定关系**。删除后 `hybridAlpha` 仍是活配置，`scan_config_liveness` 仍绿。
这件反坐实已写进台账，防止后人照旧推测重新「回滚修复」。

### 为什么「台账归零」也值得一整套判据

台账是**双向**判据（新增即红 / 清掉也要改表）。删掉后它与实测都是空集 ——
**「空对空」是一种会骗人的绿**（它永远成立）。故 `tests/v3230` 补上：
① 集合确实为空（期望状态，不是被谁清空了没人管）；② 处置台账逐条记着这 4 条（少一条即红）；
③ 保留面逐条在场（删代码最怕删错同族兄弟：`fulfillPromise` / `resolvePromise` / `hybridMerge` /
`queryByRef` / `queryByType` / `recent` 六个逐个点名）。

### 同轮修掉的三处判据自身缺陷

1. **判据在保护已被设计淘汰的状态** —— `v3156 [4f]` 断言 `countOf(…, 'numOr(this.config.config.hybridAlpha, 0.7)') === 2`
   （「RRF 路径与遗留路径都做防御式读 α」）。它把「两条并存的读 α 路径」当成了不变量，
   而那条遗留路径自 v3.50 起一次都没执行过。已改为 1（遗留路径若被误恢复 ⇒ 变 2 ⇒ 转红）。
2. **判据把「4 条存在」当不变量** —— `v3228 B1/B2/N3` 在空集合上分别退化成「空对空」与「空跑」；
   已改造为「空集合 + 台账如实记账 + 理由齐备」，并把 N3 从纯文本对照升级为**整仓镜像端到端负控制**。
3. **负控制因「自扫」变假绿（本轮实测的坑）** —— 首版 `v3230 N1` 把探针方法名
   `probeDeadMethodN1` 完整字面量写在判据里，而 A7 的扫描面含 `tests/` 全量在役 `.mjs`，
   于是扫描器把**判据自己的探针文本**当成「有一条测试引用」，
   读数变成「方法 430→431 / 仅测试 33→34 / 零引用仍为 0」—— **破坏根本没被观测到**，负控制静默失败。
   修法：探针名运行时拼接（`'probeDead' + 'MethodN1'`）。

### 本版交付

- `index.js`：删除 4 处方法定义（53 行）；3 处历史注释改述（不再把已删方法说成「仍在场」）。
- `tests/audit/scan_wiring_dead_methods.tsv`：冻结基线 → **处置台账**（`# [已删·分类] 方法名 — 理由`）。
  空集合仍是「新增零引用即红」的判据真源，不是基线失效。
- `tests/v3230_dead_method_disposal.test.mjs`（9 项）：A 处置本身（定义已删 / 保留面 / 台账格式）+
  B 基线不退化（防空对空 + 反坐实留痕）+ C 口径未退化 + **N1/N2 两条整仓镜像负控制** + D 版本锚。
- 交棒：`v3229 D1` 当版精确锚 → 下限锚；`catalog_reference_consumers.tsv` 追加 v3230 一行。

### 验证

- 定向：`node --test tests/v3230_dead_method_disposal.test.mjs` → 9/9。
- 全量：209 文件 / 1932 断言 / 0 失败；`--audit` 42/42 RC=0。
- 审计读数：A7 方法 434 → 430 / 有真引用 397 / 仅测试 33 / **零引用 4 → 0**；`scan_config_liveness` exit 0。
- 死代码预算：实测 37184 行 < 登记上界 37538（余量 354）⇒ **无需 `--bump`**（不把删除当成抬上界的理由）。

### 边界（如实声明）

① 口径仍是**文本形态**判定（不解析 AST、不追 `obj[name]()` 一类动态拼名调用），
   故「零引用」只表示文本面找不到任何引用形态；本版四条均已用 `grep` 全仓复核含字符串面。
② 删除是**破坏性不可回滚**动作（除 git 历史），已用 `tests/v3230 A2` 把保留面逐条钉住。
③ 真实 SillyTavern 宿主实机**未验证** —— 无头门禁只证明模块间契约。

## v3.228.0

**主题：O-5 性能取证 —— 先有可比基线，才谈得上优化；本轮顺带修掉探针自身一处把夹具成本算进被测调用的口径错误（两次读数差 290 倍）。**

### 口径先说清（本仓对「性能」的一贯纪律）

本版读数是**合成数据 + 真模块**：没有浏览器、没有 SillyTavern 宿主、没有真实 1000 楼聊天记录。
所以这些数字**不代表实机性能**，只用于回归时看量级是否漂移。报读数必须同时报口径与「没做什么」。

### 本轮自己的教训（值得记）

首版探针写的是 `LR.replayShift(mkHost(1000), 500)` —— `mkHost(1000)` 要建 1000 楼场景树（≈ 51.55ms），
**它被算进了被测调用**，于是报出 `replayShift 98.6ms/次`、`replayDrop 111.5ms/次`。
改为「setup 不计时、每 rep 独立 host」后是 `0.34 / 1.06ms`。**两次读数差 290 倍** ——
「先量」这一步本身也需要口径正确。这条已立成判据（`v3229 B1`：计时区间内不得出现 `mkHost(` / `buildScene(`）。

### 实测读数（合成，v3.228.0）

| 路径 | 每次耗时 |
|---|---|
| 场景书 apply ×1000 楼（建树，夹具基线） | 51.55 ms |
| 场景书 `summary()`（1000 楼，含树） | 1.53 ms |
| `replayShift` 1000 楼 | 0.34 ms |
| `replayDrop` 1000 楼 | 1.06 ms |
| `coverage(host)` 1000 楼 | 0.11 ms |
| `replayShift` 200 楼 | 0.04 ms |
| `coverage()` 200 楼 | 0.04 ms |

缩放：`replayShift` 从 0.04ms（200 楼）到 0.34ms（1000 楼）—— 5 倍楼层约 8.5 倍耗时（含一次性建账成本），
**未观察到超线性（无 O(n²) 迹象）**。

### 本版交付

- `tests/audit/perf_probe.cjs`：可复跑的探针（**setup 不计时**，输出自带 `synth: true` 声明）。
- `tests/audit/perf_baseline.json`：四面对外可读 —— `readings`（读数）/ `scaling`（缩放判断）/ 
  `corrections`（口径修正）/ `not_done`（没做什么）。
- `tests/v3229_perf_evidence.test.mjs`（9 项）：A 基线四件齐备 + **口径不得被当成实机**（正则强制
  基线里出现「合成数据 / 非实机 / 浏览器|宿主」）+ 缩放与读数自洽 + B1 **探针口径结构判据** +
  B2 行为面（真跑探针，读数与基线同量级）+ N 三条负控制 + D 版本锚。

### 同轮捐到并修掉两处判据自身缺陷

1. **探针把夹具算进被测调用**（见上，290 倍差）。
2. **负控制污染不彻底**：N2 首稿只把 `note` 改成「实机…」，而基线里 `corrections` / `not_done` 也写着
   「合成数据」⇒ A2 的正则仍能命中 ⇒ 污染没造成差异，N2 转红。修法：污染要**彻底替换所有相关字样**，
   并同时断言「失去合成数据声明」与「失去非实机声明」两件事。

### 验证

`tests/v3229` 9/9；全量门禁与 42 项审计全绿（读数见 FOUR_RELEASE_PLAN 状态检查点）。

### 边界如实声明

① 合成探针不覆盖**向量检索 / LLM 调用 / 注入预算裁剪**等 I/O 与网络路径（受宿主与外部服务影响，
   合成探针测不出意义）；② 本版**只取证与立基线，不做优化** —— 任何优化都要先有可比基线；
③ 读数受跑批机器影响，判据只拦「量级漂移」（5 倍容差），不赌瞬时抖动；④ 真实 SillyTavern 宿主实机未验。

## v3.227.0

**主题：O-6 死代码方法级扫描 —— 把「93 条疑似」复核成「4 条真死代码 + 冻结基线」。旧读数里绝大多数是 A7 自身的形态漏报，本版先把判据修准，再让数字变小。**

### 修前实测（旧 A7 的三处缺口，逐条复核）

旧读数「A7 定义但检索不到调用点 (93)」。逐条核对后，93 条里绝大多数是**扫描器自己的盲区**：

1. **不认可选调用**：`this.getNpcTiesPrompt?.()` —— 可选链在**名字之后**，而旧口径只认 `name(`；
   首稿重写时我还把它写成了「名字前置的 `?.name?.(`」，一样全漏。
2. **不认引号里的名字**：`'getPublicData'` —— 公共出口由桥按名调用，旧口径同样记为 orphan。
3. **扫描面只有 `index.js`**：`fetchModels` 与 `unlockFact` 的调用点在 **`settings-ui.js`**
   （同目录另一模块：`this.engine.llm.fetchModels(...)` / `s.unlockFact(...)`）⇒ **活代码被报成零引用**。
   判据的面漏一个文件，结论就完全反了。

一处**确实存在**的读数错位也一并记下：`breakPromise` 在旧表里**缺失**，而它其实是零引用 ——
因为旧口径的 orphan 列表被前两类误报挤满了（93 条里真有 1 条，却混在噪声里没法辨认）。

### 本版做了什么

- **A7 重写为分域台账**（`tests/audit/scan_wiring.mjs`）：按出现形态分 `call` / `opt`（可选链）/ `prop`（键名）/
  `q`（引号名）/ `test`（仅测试引用）/ `zero`（零引用），并输出 `A7.1 零引用` 与 `A7.2 仅测试引用` 两段。
- **扫描面扩到全部根级 `.js` 模块**（不再只看 `index.js`）。
- **冻结基线** `tests/audit/scan_wiring_dead_methods.tsv`：4 条零引用各自带分类理由；
  新增零引用 ⇒ 硬失败（exit 1）；基线里的名字已不再零引用 ⇒ 也要报（防基线长霉）。
  基线缺失 ⇒ exit 2（fail-closed）。
- 性能：测试面 blob **先读一次**（首稿在 434 个方法的循环里逐文件读 ⇒ 434×269 = 116k 次读盘，会把扫描器拖垮；
  实测扫描器总耗时 0.8s）。
- 新增 `tests/v3228_dead_methods_ledger.test.mjs`（9 项）：A 口径双向（可选调用/引号名/键名算引用、
  纯文本提及不算、前缀不误命中）+ A2 扫描面含 `settings-ui.js` + B 基线双向一致与「每条都有分类理由」+
  B3 fail-closed + N 三条真源码/真口径负控制 + D 版本锚。

### 修后读数（本轮交付的台账）

`方法 434 / 有真引用 397 / 仅测试 33 / 零引用 4`，零引用 4 条：

| 方法 | 分类 | 复核 |
|---|---|---|
| `_legacyHybridMerge` | 已知遗留·待决策 | 自 v3.50 引入 RRF 后无调用者（`index.js:6827` 注释自述）；`hybridAlpha` 配置键只在它里被读 —— `scan_config_liveness` 的存在理由就是它；删它需连配置键一起处置 |
| `breakPromise` | 真死·承诺账本 | 产品路径统一走 `resolvePromise(id, status)`（v3.178 起）；`fulfillPromise` 仅被 `tests/v341` 直调，`breakPromise` 连测试也没调 |
| `callGeneric` | 真死·旧生成路径 | 面向旧 `/api/v1/generate` 端点；产品路径走 `chat/completions`（`fetchLLM`）；全仓仅定义行一处 |
| `queryByFloor` | 真死·退役测试 | 唯一消费者是 `tests/archived/v354_oplog_health.test.mjs`；archived 不参与跑批 |

### 同轮捐到并修掉三处判据自身缺陷

1. **可选链写反**：`opt` 形态首稿写成 `?.name?.(`（名字前置）⇒ 真实写法 `name?.(` 全漏。
2. **探针把「定义」当「调用」**：N1 的探针直接拿整个 `index.js` 判 `fetchModels` 是否被引用 ——
   而定义行 `async fetchModels(url, key) {` 本身就能被 `name(` 命中 ⇒ 探针自证为「有引用」，
   N1 转红。修法：探针必须先排除定义行。
3. **正则转义层数写错**：`new RegExp('(?<![\w$])' …)` 在经补丁脚本落盘时反斜杠层数失真，
   扫描器当场 `SyntaxError: Invalid regular expression`。改为用字符类常量（`WORD` / `QCH`）拼接，
   不再在多级转义里数反斜杠。

### 验证

`tests/v3228` 9/9；`scan_wiring` 独立跑 exit 0（A7/A8 全绿）；全量门禁与 42 项审计全绿
（读数见 FOUR_RELEASE_PLAN 状态检查点）。

### 边界如实声明

① 口径是**文本形态**判定（`call` / `opt` / `prop` / `q`），不解析 AST，也不追**动态**调用
（如 `obj[name]()`、桥按名反射之外的拼名）；因此 `zero` 是「文本面找不到任何引用形态」，
对 `_legacyHybridMerge` 这类**已知**遗留仍靠人工分类标注，不自动判「可否删」；
② 复用根级模块面只覆盖**根目录** `.js`（子目录模块的定义本就不在 index.js 里，不属本台账面）；
③ 真实 SillyTavern 宿主实机未验。

## v3.226.0

**主题：O-4 门禁隔离与耗时 —— 先量再决定改不改；实测抓到一条真竞态（自家测试互相踩）并修掉，把隔离/耗时/仓根禁写立成常驻判据。**

### 先量（三条实测读数，不是推演）

1. **耗时构成**：`npm test` 全量 wall 37.6s、子进程 CPU 149.2s ⇒ **并行度 3.96x**（7 路测试 + 4 路审计）；
   tests 段 34.3s、audit 段 19.3s、maxrss 262 MB。audit 段最慢 8 项**全是 negctl 族**（负控制）——
   `scan_v3193_host_matrix_negctl` 10.5s / `scan_v3184_final_four_negctl` 4.8s / `scan_v3182_ledger_replay_negctl` 4.4s…
   那是「证明力」的成本，不砍。
2. **镜像瘦身是伪优化**：整仓镜像排除 `.bak` 只省 **0.2s**（0.76 → 0.56s）、体积 9.00 → 6.33 MB。
   本仓有 9 个历史 `.bak`（2.67 MB）、CHANGELOG 581 KB —— 它们不是瓶颈，**故不做**。
   段间并行（tests 段与 audit 段同时跑）也不做：负控制会大量 spawn 子进程，与 7 路测试并发叠加只会加剧资源竞争（v3.205.0 的 T7 已记录该现象）。
3. **隔离**：跑批前后对仓做快照（文件集合 + size + mtime）+ git status 对照，**0 差异** —— 隔离本身成立。

### 实测抓到的真竞态（修前是「环境资源问题」，其实是自家测试互相踩）

现象：全量并发跑批时反复出现
`⟳ tests/v3225_replay_floor_gate.test.mjs 疑似环境资源耗尽（非判据失败），第 1 次重试：at lstatSync`
`⟳ tests/v3206_process_reaping.test.mjs … at cpSync`。
长期看起来像沙箱 I/O 抖动（v3.205.0 的 T7 已把它定性为环境现象）。**本轮把它归因清楚了**：

- 本仓有多个「整仓镜像」测试（`v3206` / `v3225` / `v3159` / `v3203` / `v3204`）用 `cpSync` 递归复制整个仓库；
- 而 `v3215` / `v3221` / `v3222` / `v3223` / `v3224` 当时把临时产物落在**仓根**
  （`.tmp_v3221_n1_xxx` 等 mkdtemp 目录、`.tmp_v3215_*.js` 文件）；
- 两者在 7 路并发下互撞：镜像方正遍历到那个目录，对方 `rmSync` 掉了它 ⇒
  `ENOENT: no such file or directory, lstat '…/.tmp_v3224_n12_xxx'`。

复现路径：单独跑 `v3225` 直接报 `ENOENT … .tmp_v3224_n12_Ac7E7y`（本轮的第一次现场）。

**修法**：上述 5 个文件（共 6 处）的临时产物一律改落 `os.tmpdir()`；`v3215` 另加两处加固 ——
临时文件在 `require` 前显式删 cache key（临时路径挪位后同名复用会拿到上一轮的模块）。
修后并发复跑（受害者 + 加害者同批）**143/143 全绿**，原 ENOENT 不再出现。

### 本版做了什么

- 新增 `tests/audit/gate_timing_baseline.json`：耗时与隔离**基线**四面对外可读 ——
  `full`（全量实测）/ `subsets`（三组子集与上界）/ `isolation`（方法与实测漂移 0）/ **`not_done`（本轮明确不做什么，逐条写理由）**。
- 新增 `tests/v3227_gate_isolation_and_timing.test.mjs`（10 项）：
  · A 基线结构与「未做之事」可读性（防把「我们量了」写成「我们优化了」）；
  · **B 仓根禁写**：扫 `tests/` 与 `tests/audit/` 全部 `.mjs`（实扫 ≥ 200），
    命中「仓根 mkdtemp / 仓根 `.tmp_` 写入」即红（判据与负控制**共用同一模式常量**，见下）；
  · **C1 隔离**：跑隔离子集前后做仓快照 + git status 双向对照，必须零漂移，且墙钟不越上界；
  · **C2 耗时**：tests / audit 两组子集墙钟不越上界（拦数量级倒退，不赌 CI 瞬时抖动）；
  · D 版本锚（当版字面量）；
  · N 三条负控制：把一处 `tmpdir` 改回仓根 ⇒ B1 必须命中、破坏串必须真改源码（首稿破坏串写断了，当场被自己抓住）；
    往仓里写一个文件 ⇒ 快照比对必须抓到漂移（否则 C1 是空跑）；构造越界值 ⇒ 越界判据必须为真。

### 同轮捐到并修掉三处**判据自身**缺陷（都是新写判据自己的毛病）

1. **判据与负控制各抄一份模式 ⇒ 立刻漂移**：B1 那份「仓根 mkdtemp」模式要求字符串后紧跟 `)`，
   于是 `path.join(REPO, '.tmp_x_' + name)` 这种**拼接写法**漏检；而 N1 断言自己那份模式能命中破坏串 ⇒
   N1 当场转红。修法：抽成两个共用模式常量（`ROOT_MKDTEMP` / `ROOT_TMPWRITE`），**负控制引用判据自己的模式**，
   不是另写一份「像它」的。
2. **自扫**：B1 的扫描面含 `tests/*.mjs`，而它自己文件里同时写着「禁令模式字面量」与「正/反向探针文本」⇒
   首版把自己判成违规（两条）。修法：跳过自身，并补 `scanned >= 200` 的**防空跑**断言（豁免的代价必须用读数补回来）。
3. **给未来版本预写锚点**：D1 首稿写 `vnum('3.226.0')` 而当时仓在 3.225.0 ⇒ 版本守卫 V3 直接报
   「版本下界高于当前版（不得承诺未来）」。这不是「提前抬版」，是**把锚点写到了还没发生的版本上**；
   抬版本版时才成立。

### 验证

`tests/v3227` 10/10；受竞态修复影响的 5 个套件并发复跑 143/143；全量门禁与 42 项审计全绿
（读数见 FOUR_RELEASE_PLAN 状态检查点）。

### 边界如实声明

① 上界刻意宽（约实测 3 倍 + 15s）：本判据目标是**拦一个数量级的倒退**，不是赌 CI 机器的瞬时抖动；
② 隔离子集只覆盖 2 个含镜像/写入通道的测试（全量 5 个由 C1 的漂移判据在子集上代表，
   全量级隔离仍需在每次全量跑批后人工看 `git status`——**未自动化**）；
③ 段间并行与镜像瘦身**明确不做**（理由见 `not_done`）；
④ 真实 SillyTavern 宿主实机未验。

## v3.225.0

**主题：O-3 判据灵敏度体检 —— 审计的审计。29 个审计扫描器 × 三档退化形态全量体检，抓到 1 处真 fail-open（语法门）并修掉；把「扫描器还能不能判」从散点断言变成表 + 常驻判据。**

### 为什么做（既有覆盖的实测缺口）

本仓审计目录 29 个扫描器负责「判缺陷」，而「扫描器自己还有没有判的能力」只在测试侧散点验过：
`tests/v3159` 的 [2b] 只覆盖 3 个扫描器（`scan_resilience` / `scan_wiring` / `scan_config_liveness`）；
`tests/v3177` 的 C7 是测试里的一条断言（默认根上 `index.js` 要出现在清单里）。
**判据在测试里，不在门上** —— 门自身可以退化，而这件事没有常驻读数。

### 体检规模与三档形态（可复现，逐字写进登记表表头）

第一版夹具是错的，值得先记：把「真仓里的扫描器 + cwd=退化树」当夹具用，于是所有按
`import.meta.url` 推根的扫描器（`scan_syntax` / `dead_code_budget` / …）跑的还是**真仓**
（`HERE/../..` = 真仓），量到的是真仓通过，不是退化树通过。改为**整仓镜像 + 从镜像执行**。

- **H 正常树**（镜像零改动）—— 必须 exit 0；非 0 就是健康树上误红（假红）。
- **G 根目录 .js 全部删除**（入口不在场）。
- **T tests/ 下（除 audit/ 与 _audit_lib.mjs）所有 .mjs 掏空**（被扫描的测试面退化）。

实测结果：**H 列 29/29 全为 0**（无一误红）；**G、T 两档皆 0 的 0 个**（无恒绿探测器）。
逐条读数落 `tests/audit/audit_scan_probe_matrix.tsv`。

### 修前实测到的真缺陷（1 处，已修）

`tests/audit/scan_syntax.mjs`（语法门）：把根目录 .js 全删（或全掏空）后仍 **exit 0**，
报「270（掏空时 335）个文件均可解析」。0 个源文件的世界里「都能解析」为真，
但这与本门存在的唯一理由无关 —— 文件头写着：`index.js` 被改出结构性错误时回归全绿、
而插件在浏览器里根本不加载。旧代码只有 `if (total === 0) exit(2)` 一道，
而掏空/删除根 .js 后 tests/ 树里仍有 200+ 个 .mjs，`total` 远大于 0，那道兜不住。

**修法**（只作用于**默认根**，显式 `--root` 的语义逐字不动）：入口不在场（缺 `manifest.json`
或 `index.js`）⇒ exit 2；入口退化（低于字节下限）⇒ exit 2。失败码 2（结构漂移 = 没得判）
与 1（真语法失败 = 判出坏了）分开，读者一眼可辨。显式 `--root` 是负控制 / 等价性对照 /
小树夹具的通道（`v3177` 的 C4/C5/C6 依赖它），故守卫不碰它 —— 这正是一条「同一门两种用法，
守卫只能加在其中一种上」的取舍。

### 体检的其余实测结论（不修，如实记账）

- **`dead_code_budget.mjs` 在 T 档仍 0** 且这不是恒绿：它读根 `.js` 行数与 `manifest`，
  tests/ 掏空与它的输入面无关，且它在 G 档 exit 1（「余量过大：上界与实测脱节」）—— 已自证。
  它同时是**库文件**（`measureActiveLines` / `readBudget` 被 `tests/v3116` import），
  按仓内规范这类文件不加目录级守卫。
- **13 个负控制脚本**（`scan_*_negctl.mjs`）都从真仓 `process.cwd()` 推根，再显式设
  `LONSHA_AUDIT_ROOT` 指到自己的 fixture；`v3184` / `v3185` / `v3186` / `v3193`×2 这 5 个
  还会把被测扫描器复制进 fixture 的 `tests/audit/`（因为被测量从自身目录推根）——
  两种形态都是对的，本版不动。

### 常驻判据（本轮立起来的那条）

新增 `tests/v3226_audit_sensitivity.test.mjs`（14 项）：
登记表结构与可复现性（表头须逐字写明三档形态）/ **双向齐全**（在役扫描器与登记项互为全集，
新增不登记 ⇒ 红，退位不摘 ⇒ 红）/ 每个负控制的被测目标都在表内 / H 列全 0 /
「没有恒绿探测器」（G、T 至少一列非 0）/ D 组把语法门的新守卫**行为化**（最小镜像上跑
默认根与 `--root` 两条路径：入口删/掏空 ⇒ 2、健康小树 ⇒ 0、损坏小树 ⇒ 1、不存在 ⇒ 2、真仓 ⇒ 0）/
N 组三条真表破坏负控制（H 改 1 ⇒ J2 红；G、T 改 0 ⇒ J3 红；摘一条登记 / 加一条幽灵 ⇒ J1 双向红）。

### 验证

`tests/v3226_audit_sensitivity.test.mjs` 14/14（另跑 `v392` / `v3177` / `v3159` 三套件 1+1+12 全绿，确认语法门改动无回归）；
全量门禁与 42 项审计全绿（见 FOUR_RELEASE_PLAN 的状态检查点）。

### 边界如实声明

① 三档只覆盖「输入面不在场」这一维；「输入在场但内容被换掉」不在本次形态内
（那属于各扫描器自己的负控制面，13 个已有）；
② H/G/T 是**登记基线**，不是「这些数字永远正确」—— 判据读的是表本身（结构 + 齐全 + 灵敏度三条性质），
表里的读数更新由体检脚本重跑产生；
③ 真实 SillyTavern 宿主实机未验；
④ 本版**不改** 8 个无负控制驱动的扫描器（`dead_code_budget` / `scan_audit_lib_consolidation` /
`scan_bridge_reader` / `scan_claim_truthfulness` / `scan_config_liveness` / `scan_cross_repo_binding` /
`scan_process_reaping` / `scan_syntax`）—— 它们已由 H/G/T 三档与 `v3159`/`v3177` 兜底；
「为每个扫描器各写一份负控制」是另一件事（O-6 之外的新项，未登记进本轮范围）。
...
R3-A（v3.220.0）把层级树 / 到访史 / 本楼场景头接进了 `summary()` 并外供到手机端。但「能写出去」
不等于「回滚之后还成立」：本轮逐条实测七项真缺陷，全部落在**读数撒谎**这一类上（说「那天 8月2日 暴雨」
而那一天已被删掉）——比缺读数更糟。

### 修前实测（在真模块上跑出来的，不是设计洁癖）

1. **`import()` 把「没给」编成第 0 楼**：全路径用 `num(x) ?? 0`，而 `Number(null) === 0`、`Number('') === 0`，
   于是 `nodeFloor / updatedAt / trackFloor / opsFloor / visitFirst / visitLast / presAt` 七格
   「没给」全被读成 **0**；`headers` 的 `[null, ...]` / `['', ...]` 键同样静默落成第 0 楼。
   0 是**合法楼层**，与「没给」同形最贵。
2. **`setHeader(null)` / `headerAt('')` 读成第 0 楼**：`setHeader(null, {date:'X月X日'})` 返回 `true`
   并真的写进第 0 楼；`headerAt('')` 答得出第 0 楼的场景头 —— 与该方法「没有就 null」的自述直接矛盾。
3. **删楼：场景头一格不动 + 在场被整表全清**：删掉第 7 楼后 `headerAt(7)` 照样答
   「8月2日 · 暴雨」（读数指向一个已不存在的时刻），而宿主在回滚后无条件 `clearPresence?.()`
   把在场**整表清空** —— 第 9 楼那批人的所在一起没了：单楼语义被悄悄扩成全清。
4. **前移：三面里只有一格跟，且不认「被删楼自身的残留」**：`headers` / `presence` 一格不动；
   第 8 楼前移成 7 楼后与残留**撞成两条 7 楼**（连做两次得 `track=[3,7,7]`）——
   登记项自述的「drop/shift 必须幂等」在 shift 侧从未成立。
5. **导入旧格式存档后第一次单楼编辑把整棵树清成 0 个节点**：`nodes.clear()` 之后「按 opsLog 重放」，
   而旧档没有 `opsLog` ⇒ **没有人重建**。导入存档恰恰是最需要保住数据的地方。
6. **`coverage()` 不含任何场景头读数** ⇒ 删楼/前移对 `headers` 的处理**没有任何判据面**：
   改坏了也看不出来。

### 本版做了什么

- **判开「没给」与「给了 0」**（延续 R3-A 的 `numOrNull`）：`import()` 七格 + `headers` 键、
  `setHeader` / `headerAt` 的楼层取值口径统一改走 `numOrNull`；`setHeader(null/'')` 如实拒绝写入。
- **回滚面真认三面**：新增 `clearHeader(floor)`（单楼撤场景头）与 `shiftFloorRefs(deleted)`
  （先清**被删楼自身的残留**，再平移 `track` / `opsLog` / `headers` / `presence` 四面）；
  `rollbackFloorOnly` / `rollbackFrom` 按同一语义撤场景头（单楼 / 级联）。
- **粒度收回到模块内**：`ledger-replay.js` 的 `scene` 登记项不再手抄循环（手抄的那份必然漏掉后来新增的面），
  drop 走 `rollbackFloorOnly`、shift 走 `shiftFloorRefs`；宿主 `index.js` 删掉 `clearPresence?.()` 全清一句，
  按楼层清的职责归模块（**只清停在该楼的人**）。
- **重建不缩水**：`_rebuild` 在**无 `opsLog` 真源**时不再清空式重建（如实降级：只摘掉被删那楼的登记），
  未删楼的登记与到访史保留。
- **补判据面**：`coverage()` 增 `headerFloors` / `headerCount`（场景头楼层列号可逐楼对账）；
  场景头上限提为常量 `MAX_HEADERS`（写侧同时改用它，**载入侧此前无上限**）并进导出面。

### 同轮接管的既有判据（收紧，未放宽）

- `tests/v3181_spatial_grounding.test.mjs` 的 E2 原本把「宿主 `clearPresence?.()` 全清在场」写成断言 ——
  那句全清正是本版要修的粒度问题。改为两条**更强**的判据：宿主**不得**再全清，且按楼层清必须由模块承担。
- `tests/audit/scan_v3181_spatial_grounding.mjs` 的 N1「登记表 scene 面」锚点随语义搬家
  （`h.scene.rebuildFromOps()` → `h.scene.shiftFloorRefs(d)`），并新增两组接线判据。

### 验证

- 新增 `tests/v3222_scene_rollback_closure.test.mjs`（24 项：A 没给不再编 0 / B 回滚与前移真认三面 /
  C 重建不缩水 / D 有界性 / E 只读 / F 负控制五组真源码破坏 + 互不掩护 / G 接线与退路同形 + 版本锚）。
  负控制走「真源码破坏 → 破坏副本 → 同款真判据必须转红」，形态判据一律先剥注释
  并**先切掉 `clearHeader` 方法体**（不切会被同名片段恒真兜住）。
- 全量门禁与审计见下方交棒记录。

### 边界如实声明

- 本版只改**回读/回滚/重建**三条路径，未改场所图景的对外语义（`count` 仍是「去过的不同楼层数」、
  `depth` 仍是真实层级、三面的键面与 R3-A 一致）。
- 重复调用 `shiftFloorRefs` 仍会**再次平移**（平移语义，不是 drop 侧的幂等契约）；
  登记项自述的幂等只在 drop 侧成立。
- 真实 SillyTavern 宿主实机未验：无头门禁只证明模块间契约。
...
### 修前实测（真源码重放，不是设计洁癖）
- **双向关系**：关系边是单向主观的（提取提示词第 3 条明写「A看B 与 B看A 分别各记一条」），
  而 `buildInjection` 的 `[角色关系]` 块只把召回边原样列出 —— **没有任何一格说「对侧那条在不在」**。
  于是「乙对甲是警惕」与「乙对甲从未登记」在注入面上**同形**，而后者是提取漏了一条（该补记）。
  更细：「对侧不在」有三种来源（本轮被披露条件挡下 / 曾经存在但已失效 / 压根没登记），
  处置方向互不相同，压成一态就会让模型去补一条不该补的关系。
- **知情网络**：`markUnaware` / `revealKnowledge` 用 `includes` / `!==` **逐字比较**，
  而 fact 是 AI 每轮重新生成的自然语言，措辞必然逐轮不同。逐字重放真源码：
  同一件事三种措辞（「地下室魔法水晶被神秘黑影盗走」/「魔法水晶被盗」/「帕秋莉的水晶被偷了」）
  记 **3 条**；用「博丽灵梦告知了水晶被盗的事」去解除，**一条都清不掉**。
  两个后果都是致命的：`getReEntryNotice` 只取前 3 条，前 3 格被同一件事的旧措辞占死，
  真实新增的认知边界**永远挤不进去**；以及**认知隔离永不解除** —— 角色已经被明确告知了，
  系统还在每轮注入「切勿未卜先知」，把已经知道的事当成绝不能说的事。
### 收口
- 新增 `relation-mutual.js`（双向关系对账，只读、不抛、不猜）：
  收**两份**输入（图里的全量边 + 本轮实际进注入的那批），输出四态 ——
  `mutual`（对侧也在本轮注入）/ `mutualGated`（对侧在图里、本轮被挡）/
  `mutualExpired`（对侧已失效）/ `oneSided`（对侧压根不在图里，**唯一该补记的一态**）。
  注入渲染只对 `oneSided` 追加「〔对侧未登记·勿当对称〕」：被挡/已失效两态标了反而误导。
  清单有界且**截断可见**；输入不是数组与「空图」分开报（`malformedInput`）。
- 新增 `knowledge-network.js`（知情网络，只读判定、不写库）：
  同一性三级判据（归一逐字相同 / 互为子串且短侧 ≥ 4 字 / bigram 重合率 ≥ 0.75 且短侧 ≥ 6 字），
  **判不开一律判不同**（误合并 = 角色提前知道他不该知道的事，且在读数上完全看不见）。
  另加一档 **`near`（疑似）**：公共二元组 ≥ 2 时只报候选、不合并 ——
  实测的告知式措辞重合率只有 0.17，任何不冒误合并风险的阈值都判不出「同一件事」，
  修前这件事与「没有同事实」**同形**，于是「永不解除」看起来像「本来就没有要解除的」。
  `reconcile` 的解除动作两步**互相独立**（清未知侧 + 已知侧幂等）：
  写成二选一会在「已知侧已有同事实」时把未知侧漏掉，那正是本模块要治的病。
- 宿主接线：`markUnaware` 同事实不重复登记、已知侧有同事实不再登记为「不知道」
  （修前两条相反指令可以并存）；`revealKnowledge` 按下标删（值不等于措辞），
  疑似不删只计数；模块缺席回落逐字口径（数据不丢）并记入读数。
  累计读数落在 `worldProg._knowledgeRead`（**累计而非本轮**：单轮的「解除 0」与「本轮没有揭示」同形）。
- 诊断面：`selfCheck` 增「双向对账」「知情网络」两行。
  报警只认真损失 —— 对账行只认 `oneSided > 0`（被挡/已失效是正常态），知情网络行只认 `suspect > 0`。
### 测试
- 新增 `tests/v3220_relation_mutual_knowledge.test.mjs`（17 条）：四态互不相同且各落各的格、
  截断可见、畸形与空图分开报、同一件事不堆积且措辞漂移可解除、两件不同的事不误合并、
  告知式措辞进疑似档（带候选）、已知侧拦截、解除两动作独立、宿主真接线、诊断行真进 rows；
  负控制四条（真源码破坏 → 载入破坏副本 → 在副本上重跑**同款判据**）。
- 既有 `v341`（认知隔离动态）与 `v3184`（关系披露）全绿：新判定是旧逐字口径的超集，逐字相同仍逐字命中。

## v3.218.0

**主题：R2-E 取消留痕 —— 「被中止」与「正常完成」不得同形。**
这是 R2 范围里「取消和迟到隔离」的**取消**那一半（迟到那半由 R2-B / R2-D 收口）。

### 修前实测（真源码读取，不是设计洁癖）

- `GENERATION_ENDED`（用户 Esc 中止）在当前实现里只做一件事：复位
  `_generationActive = false`（v3.12 的兜底，防自愈调度器永久延后）。
  于是读数上两种处境**同形** —— 中止那一轮同样有 `_lastInjection`
...
- 读数增 `outcome`（缺省 `'pending'`；读数键面 10 → 11）。
...
- 两处事件接线：`_h1`（MESSAGE_RECEIVED，在 `onMessageReceived` **之前**落结局）
  与 `_h6`（GENERATION_ENDED，在复位标志**之后**、与标志互不依赖）。
- 展示面：`buildInjectionReadout()` 带出 `outcome`（外供面 9 → 10 键）；
  `selfCheck` 的注入读数行增「结局」与完成/中止计数；注入预览面板加结局徽标
  （✓ 已完成 / ⚠️ 被中止，零块分支同样带结局）。

### 跨仓纪律（本轮特别标注）

`outcome` 一旦进快照外供面，**下游 `ruby-phone` 的 `config/injection-contract.js`
必须同一轮接上**（读它、把「被中止」与「正常完成」分开报）——
否则又是一次「上游给了没人读」，即本仓已点名的第七次形态。
故本轮两仓**同批**推进：上游 v3.218.0 + 下游 v3.0.3。

### 边界如实声明

本 Gate 只保证「结局三态可分且幂等」。**不声称**覆盖宿主不发 `GENERATION_ENDED`
的情形（那种情况下结局停在 `pending`，面板明写「结局未定」而非硬猜成某一态）。

### 验证

- `tests/v3219_outcome_truth.test.mjs`（11 条，先红后绿；负控制真源码破坏 → 载入破坏副本
  → 重跑同款判据）；全量见本轮门禁输出。

## v3.217.0

**主题：R2-D 无主暂存不得被别的代捡起 —— R2-B 已经**如实声明**的那条边界的另一半。**

R2-B（v3.216.0）治的是**同一条路径内**的迟到：`onBeforeGeneration()` 在 await 内部落地读数，
而代际守卫在 await 之后判定。本版治的是它旁边那一类：**载荷由 A 路径留下、被 B 路径提交**。
...
- **`_injectionCommit(myGen)` 只核对「调用者传进来的代」与当前代**
  ...
  落成读数 —— 读数的 `round` 照常推进，而 `gen` 却是上一代那个值。
  这正是 R2-B 注释里点名过的「张冠李戴比不落地更坏」，只是来自另一侧：
  那里是「过期分支没清暂存」，这里是「**根本没人提交它**」。
- **`window.lonsha_memory_interceptor`（保留的兼容发布路径）从不收尾**：
  它调 `onBeforeGeneration()` 之后只写注入槽位，既不提交也不丢弃 ⇒
  暂存永久悬空，成为上一条的现成供体。而该路径在真宿主上确实会被走到
  （本文件原注释：「部分 ST 版本通过 manifest generate_interceptor 调用」）。

### 收口

- **载荷自身代核对（`_injectionCommit` 的第二道门）**：`p.gen !== gen` ⇒ 不落地，
  并按过期收尾（清暂存 + 留痕）——既不落成读数，也不静默消失。
  （`Number.isFinite` 守卫：语义与 `_injectionStage` 的 `gen` 缺省一致 ——「无代」视同当代，
  旧载荷 / 手搓对象不被误杀。）
- **收尾唯一入口 `_injectionClose(myGen)`**：一条发布路径本次生成的收尾只有这一个入口 ——
  有暂存则提交；提交不成（代不符 / 无主载荷）即按过期收尾，且**不重复留痕**。
  为什么单列而不让调用方自己拼：收尾的正确性（不重复计数、不留下无主暂存）属于
  **引擎不变量**，两条路径各抄一遍正是本仓 v2.97 之前「同一口径被抄 7 份」的老形态。
  语义三条互斥且穷尽，`had` 在提交**之前**读（提交成功会清空暂存，事后读就分不清
  「本来就没有」与「刚刚提交掉了」）。
- **两条发布路径共用它**：`GENERATION_STARTED` 事件处理器与 `lonsha_memory_interceptor`
  各自占一代（`_genSeq` 递增）并各自收尾，于是「谁的载荷谁提交」。

### 边界如实声明

本 Gate 只保证「每条发布路径的读数只属于它自己那一次」与「无主载荷不得被另一路径捡起」。
**不声称**解决「两条发布路径同时活跃时谁赢」（那是宿主集成面，取决于 ST 版本实际走哪条路），
也不声称消除注入槽位之外的其它迟到写（charMem / 各账本写入）。

### 验证

- `tests/v3218_cross_path_staging.test.mjs`（11 条，先红后绿；负控制一律真源码破坏 →
  载入破坏副本 → 重跑**同款判据**）；全量见本轮门禁输出。

## v3.216.0

**主题：R2-B 迟到隔离 —— 读数落地不得早于代际确认。这是 R2-A 已经**如实声明**的那条边界。**
R2-A 把读数收口到唯一构造点 `_injectionRecord`，但**写入时机没动**：`onBeforeGeneration()`
在 `await` 内部**无条件**落地，而代际守卫 `if (myGen !== this._genSeq)` 在 await **之后**才判定。
快速连发两次生成时，先发那一轮在 await 期间已经把读数写进去了；守卫随后只拦住**注入槽位**
（`writeInjectSlot`），拦不住它早已写脏的读数。于是面板上「最近一次实际注入」可能是
**一次从未生效的注入** —— 而它旁边那行写槽位的结果恰恰说明这轮没生效，两行读数互相矛盾。

### 收口

- **`_injectionStage(payload)`（新增）**：生成路径在 await 内部只**暂存**（`_injectionPending`），
  **绝不动 `_lastInjection`**。「AI 真实所见」只能由代际确认过的提交写。
- **`_injectionCommit(myGen)`（新增）**：读数的**唯一落地点**，在守卫 `if` 块**之后**调用
  （`GENERATION_STARTED` 处理器内）—— 「记录发生在 await 之后」是**结构性**保证，不是约定。
  内部再做一道代际核对：即便被误调（守卫被绕过 / 方法被摘下来单用）也**不静默落别的代的载荷**。
- **轮次号只由提交推进**：被丢弃的那一代**不占号**，于是「第 N 轮」恒等于
  「真正生效过的第 N 次注入」。修前后者会被空号稀释，而下游把 `round` 当有效注入次数用。
- **过期分支必须清掉暂存**：不清则下一次新鲜提交会**捡起上一轮的载荷**落成读数 ——
  比「不落地」更坏：那是一份**张冠李戴**的读数，而读数上看不出来。
  同时把被丢弃载荷的读数也记进 `_lastInjectionDiscard`
  （`pendingDiscarded` / `payloadChars` / `payloadBlocks`）——「过期丢了多大一块」可读，
  而不是只有一个计数。
- 全源码 `_injectionRecord(` 的调用点**只剩提交一处**（结构判据守）。

### 边界如实声明

本 Gate 只保证「读数落地不早于代际确认」与「过期载荷不得被下一轮捡起」这两件事。
**不声称**消除了注入槽位之外的其它迟到写（如 charMem / 各账本写入）—— 那些各有归属，
属后续 Gate 范围。

### 仪式

- 新增专项测试 `tests/v3217_stale_isolation.test.mjs`（11 条，先红后绿）。
  负控制一律**真源码破坏（锚点恰中 1 次）→ 载入破坏副本 → 在副本上重跑同款判据**。
- 版本四源同步（`index.js` / `manifest.json` / `package.json` / CHANGELOG 顶节 = 3.216.0）。
...
一张可查对账表，并给「撤销一条错误事实」补上它在用户面前从来不存在的那道入口。**

本轮两件事同属 R1（业务投影 / 工作台 / 证据修复）的收口：**先能查（读面），再能改（写面）**。

### R1-E 九账只读对账面（新增 `evidence-workbench.js`，413 行）

修前实测（真跑取证）：九本账各自有 `list()` / `summarize()`，但对**外**是一排孤立文本行——
`ledger-replay.js` 的 `FLOOR_OWNERS` 只登记「谁有楼层归属」，不含条目正文、状态与引用键；
快照 15 字段里**一本账都不带**（`worldProg` 只带四本账的原始状态，三面账连状态都没进）。
下游要回答「她现在住哪里 / 这个承诺是哪一楼说的」只能逐 App 翻，且**拿不到出处**。

- `LEDGERS` 登记表是**单一真源**（九项，各声明 id / label / 取状态 / 取账本 API / 引用前缀）。
- **三态读数不得压平**：`ok` / `empty` / `absent`，absent 再分 `module-unavailable`（模块没挂）
  与 `state-missing`（状态取不到）——三者处置相反，同形即误导。
- 出处投影：每条带 `ref`（稳定引用键）+ `ledger` + `floor`。**楼层取不到一律 `null`，绝不写 0**
  —— `Number(null) === 0` 会把「没给」与「第 0 楼」两个相反读数塌成同形。
- 实测当场抓到两处真缺陷（不是读源码推算出来的）：① event 条目**没有 `status` / `floor`**，
  只有 `abandoned`；照抄别账写 `it.status` 会恒为空串，而「空」与「没弃」语义相反 ⇒ 改为
  按 `it.abandoned` 如实两态输出 `'abandoned'` / `'open'`；② echo 条目**没有 `id`**，
  初稿的 ref 回落成正文（`echo:今天风很大`）——把**正文**当稳定键，正文一改下游就定位不到
  ⇒ 新增 `spec.keyOf` 机制，echo 按其自身去重口径（`char + mode`）生成键，ref 变成 `echo:苏晴/pocket`。
- `search()` 四态分开（hits / missLedgers / emptyLedgers / absentLedgers，「这本账里没有」≠
  「这本账不存在」）；`findRef()` 五态（ok / bad-ref / unknown-ledger / not-found / ledger-absent）；
  `line()` 诊断行把缺席逐本**带因**写出。

### R1-F 修复预览与受控写入面

- **`preview(input)`（新增纯函数）**：与 `request` 共用 `validate()` 与 `affectedBy()`，故两边算出的
  affected 逐条一致（预览过 ⇒ 登记也会过同一清单）。**签名里刻意没有 rawState** —— 谁想塞状态进来
  也塞不进来，从结构上保证只算不改。
- **`request()` 幂等**：`dedupeKey` 同键且**未放弃**⇒ 原样交回既有那条并标 `replayed:true`
  （不新增记录）。**只按「未放弃」判重**：用户放弃过后重试是**新意图**，按全部历史判重会把
  「改主意后又想做」永久锁死。缺键时行为逐字同旧版（不放宽也不收紧既有调用方）。
- **`copyRepair` 增 `dedupeKey`（纯增量，不抬 `RL_VERSION`）**：旧存档读回空串即「没给」，
  不构成结构代际变化；抬版本号会让旧档无谓地走一遍迁移。
- **`requestRepair` 透出 `replayed` / `changed`**：幂等重放与真登记在下游看起来一模一样
  （都是 ok:true + 同一条 id、同一个 total），吞掉这两个字段等于把「这次什么都没写」伪装成「这次写了」。
- **桥分两面**：只读面（`version` / `bridge` / `snapshot` / `sourceState` / `lastError` / `refresh()`）
  契约不变、逐字未动；受控写入面**只收在唯一命名空间 `repair`** 之下
  （`preview` / `apply` / `settle` / `abandon`）。不肯摊在顶层：那会让「读一份快照」与「改一本账」
  共用同一个扁平命名空间，调用方从名字上分不出哪一下有副作用。
  *既有只读键面判据（tests/v388）同步**收窄**为「除 `repair` 外顶层零写语义键」，并加判「写面只在
  这一个命名空间下」——不是放宽，而是被指向唯一写命名空间。*
- **写入过两道门，任一不过即拒且账不动**（`apply` / `settle` / `abandon` **三个写动作都要过**）：
  ① `idempotencyKey` 必填（缺 / 空白 ⇒ `missing-idempotency-key`，仅 `apply` 需要）。内部 API 缺键时
     **本来就不幂等**（旧行为逐字保留），写入口必须比它严一档。
  ② `expectRevision` 必填且必须**等于**当下修订号（缺省不得被当成 0 而"恰好"通过），
     不等 ⇒ `revision-mismatch`。回滚 / 恢复 / 切聊都会推进变更栅栏号，此刻手上算好的 affected
     清单来自**旧代数**——照写就是往新会话里塞一条按旧状态算出来的修复。宁可拒，不可错位写入。
     *为什么 `settle` / `abandon` 也要过这道门（逆向审计补口）*：记录 id 是**每条 state 自己的 seq**
     （`rp_1`），切聊后新会话的 `rp_1` 与旧会话的 `rp_1` **同号**——一条陈旧 settle 会落到新会话那条
     同号记录上把它标成 done。表现是「落定成功了」，但落定的是别人的修复（v3215 5b 逐条锁住）。
- **回执形状恒定 9 键** `{ok, reason, repairId, action, affected, total, revision, replayed, at}`：
  唯一构造点 `repairReceipt()`，失败分支与成功分支同形（少写三个键，「字段缺失」就再次与「字段是空」同形）。
  `revision` 一律回**写后**修订号；`total` 在四个方法里语义一致（= 本条修复的派生件总数）。
- 新增 `engine.repairRevision()` 作 `_mutationEpoch` 的**唯一对读口**：不这么收，
  一个私有字段就成了对外契约的一部分，改名即悄悄失配，而失配的表现是「写入被拒」，没人会想到是改名。
- **边界**：本面**不自动改派生件**（改哪一处是产品决定，自动改会累积幻觉删改）。
  人工点「纠正归属」≠ 正文已发生；回执只记「账上落定了什么」，不推断剧情已经发生。

### 验证

- 新增回归 25 项（`tests/v3214` 11 + `tests/v3215` 14），全部**先红后绿**；
  负控制一律真源码破坏 + 载入破坏副本 + 在副本上重跑同款判据（N1 链式拆门①、N2 丢掉「未放弃」限定、
  N3 回执漏键、N4 拆 settle 修订门），工具自证锚点不存在 / 不唯一 / 同值替换必须抛；
  N4 的锚点**特地带上一行 settle 独有注释**——裸行 `if (i.expectRevision == null || ...)` 在
  settle 与 abandon 各出现一次，不唯一的锚点会被另一处掩护住，判定假绿。
- 全量门禁：**195/195 文件、1707 断言 0 失败、42/42 审计 `RC=0`**（测试段 34.2s、审计段 18.2s）。
- 版本四源同步（`index.js` / `manifest.json` / `package.json` / CHANGELOG 顶节 = 3.214.0）；
  资源清单冻结计数 13→14（`evidence`）、`extra_js` 61→62、死代码预算 ceiling 35749→36025（实测 35625）。
