## v3.184.0
**调研清单收尾四件套（关系披露 / 图谱汇总 / 宽容填充 / 行级变更集）：把四项剩余移植全部落地，并顺手修掉三处「接上了但运行时永不走」的失效**
...
滚动下界字面量一并推进。新增三份版本专属测试共 **53 项**：
`v3184_relation_disclosure`（18）/ `v3184_fuzzy_patch`（17）/ `v3184_changeset`（18），
