...
`presence` 一直逐人记着「谁在哪一楼」，但**没有任何出口回答「同一层楼里是不是有两个以上的人」**
—— 而那正是剧情冲突的**事实前提**。下游（手机端「地点」App）要做这个读数，
只能自己遍历 presence 再按地点分组 —— 那就是「同一口径被抄 N 份」的种子
（本仓 v2.97.0 收敛过一次同形缺陷）。

新增 `coPresence(floor)`：

- 返回面只有 **楼层 / 地点键 / 路径 / 在场名单 / 人数** ——
  **没有任何「冲突激烈程度」字段**。账本能回答「两人此刻都在钟楼」，
  不能回答「他们会不会打起来」；把后者塞进读数就是拿猜测冒充事实（本仓最贵的那类错读数）。
- 口径细节：`atFloor` 为「没给」的人**不进任何一层楼**（不会与别的「没给」凑成一组，
  且 `skippedUnknownFloor` 如实计数）；名单与地点键**同一口径**（拼音序）稳定排序；
  只读、有界（`MAX_CO_PRESENCE_ROWS = 120`），**`count` 是截断前的真实条数**（不许被截断改写）。
- `summary()` 带上 `coPresence` ⇒ 下游快照自动获得它，且获得 `meta.fieldTypes` 三态自述。

### F-6：两条观察项的口径自述

新增 `observationNotes()`，把两条**一直只写在注释与文档里**的观察项变成**机器可读**的自述
（下游可以在自己的诊断页如实转述 —— 那才是「如实声明」真正生效的地方）：

| id | 口径 | 为什么不改 |
|---|---|---|
| T17 | `shiftFloorRefs` 重复调用会再次平移 | 模块内加去重需要「记住哪些楼层已平移过」，**那份记忆会成为第二个真源**（回滚/重建后与 opsLog 不一致 ⇒ 比重复平移更坏的错读数）。调用方职责：一次删楼只调一次。 |
| T16 | 宿主未发 `GENERATION_ENDED` 时注入读数如实停在 pending | 不猜结局：「没有结局消息」既不是「已完成」，也不是「被中止」。调用方职责：需要结局时自行判超时。 |

本方法**纯读、无副作用**（不改任何计数、不改任何行为），只是把已知边界说清楚 —— 由套件 A3 钉住。

### 本版探针当场抓到一条真缺陷（三态塌成两态的新形态）

`coPresence` 首版写 `const want = (floor === undefined) ? null : numOrNull(floor)`，
于是 `coPresence('没给')` 与 `coPresence()` **同义** ——
**「给了但解不出」被当成了「没给」**：调用方给了个解不出的值，读数从「空」变成「全量」，
而它看不出差别。这与 O-1（`rollbackFrom(null)` 不得被读成第 0 楼）是同族，新形态是
**给了但解不出 ≠ 没给**。

修法：用入参个数判「没给」；给了但解不出 ⇒ 如实**返回空**并带 `rejected: 'floor-unparsable'`
（拒绝，而不是当作没问）。而真给第 0 楼仍照常按第 0 楼算（0 是合法楼层）。

### 落地物

- `scene-book.js`：`coPresence()` / `observationNotes()` / `MAX_CO_PRESENCE_ROWS`（经 api 导出）
  + `summary()` 两面
- `tests/v3233_co_presence_and_notes.test.mjs`（15 项，含三条真源码破坏负控制）

### 验证

定向 `v3233` **15/15**；全量见下节门禁结果。

### 边界如实声明

① **只给事实**：不猜关系、意图、冲突激烈程度与后果（由 A2 在字段面与源码面双钉）；
② F-6 **只外供口径，不改口径本身**（T17 的重复平移仍然会发生 —— 那是调用方职责）；
③ 未验实机（宿主真实写入 presence 的路径不在本版覆盖范围）；
④ `coPresence` 的「同刻」由 `atFloor` 表达（同一楼层号 = 同一时刻的读数），
   不引入时间戳比对 —— 账本里在场记录的时刻语义由宿主决定，本模块不替它解释。

## v3.231.0

**主题：P-2 index.js 宿主巨兽分诊 —— 把「16922 行」从一句话变成读数，并按读数否掉「按域拆走一块」。
本版只量、不拆：`index.js` 一行未动。**

### 为什么要先分诊

`index.js` 16922 行（占全仓 46%）此前只有「它是巨兽」这一句话。
没有读数就动一个 16922 行的宿主文件，是拿猜测改架构 —— 而它与 O-5（性能先立基线）、
P-3（快照先量再定）是同一条口径纪律。
...
|---|---|
| 总行数 / 成员数 | 16922 / **559** |
| TOP5 成员合计 | **3668 行 = 21.7%**（onMessageReceived 1217 · selfCheck 694 · buildInjection 629 · recallMemory 625 · onBeforeGeneration 503） |
| TOP40 合计 | **8684 行 = 51.3%** |
| 10 条前缀域规则覆盖 | **4174 行 = 24.7%**（剩下 75.3% 落在规则之外） |
| over-80 行成员 | 32 个（低耦合非生命周期 18 / 高耦合非生命周期 10） |
| 缝合模块接线 | **34 个模块 / 45 个引用点** |

形状：**少数超大成员 + 长尾小成员**。

### 结论：按读数否掉「按域拆」

① **最大的成员是「生命周期接线本身」**：`onMessageReceived` 1217 行 / `onBeforeGeneration` 503 行 /
`registerEvents` 368 行 —— 搬它们＝把入口从宿主事件上摘下来。而它们的**外部引用数极低**
（`onMessageReceived` 仅 1 次）：这是「**低外部引用 ≠ 低耦合**」的典型误读面 ——
引用数只看别人叫不叫它，看不见它是谁的**唯一入口**。故分诊表专设 `is_host_lifecycle` 一列把这类排除。

② **引用读数只能当上界，不能当判据**：同名方法在不同类上出现多次（`import` 4 处 / `constructor` 3 处），
按名字聚合会把它们混在一起（`import` 合计 27 次外部引用，那不是某一个方法被叫 27 次）。
要精确必须做 AST 级归属，而本仓死代码口径**明确是文本形态、不解析 AST**（O-6/P-1 已立）。

③ **内部互调稠密**：规则外成员内部互调 **4696** 处 —— 拆一个域要同时改它与其余全部调用点；
而本仓 9 道门禁里**没有任何一道**检查跨模块成员可见性，拆错的形态是**运行期 this 丢失**
（只有实机看得见，无头门禁看不见）。

**唯一有读数支持的第一刀不是「按域拆」，而是「先立成员级预算 + 文件规模上界」** ——
本版把这件事立成判据（含每个大成员的逐条在场点名），**不动手**。

### 同轮口径留痕（两处「读数看起来也像读数」的教训）

- **探针首版把缩进层级搞错了**：按 4 空格切分成员 ⇒ 只切出 **4 个方法**、89.5% 落进「未归类」——
  那是把 4 空格层级的 `if (…) {` 与 `const X = …` 当成了方法签名。改为 8 空格（类体 4 + 成员 8）
  并显式排除控制流关键字后，切出 **559 个成员**。**读数错的时候它看起来也像读数**（一张漂亮的
  89.5% 占比表），故套件 B1 专设「成员数须落在合理量级」的自证判据。
- **缝合模块接线面首版算出 0**：用「遇右括号即止」的匹配，而真实调用是
  `_moduleLib(() => window.LonShaXxx, 'scene-book.js')` —— 第一参箭头函数里就有 `)`。
  改正后为 34 个模块 / 45 个引用点。**假零比假高更危险**：它安静、且看起来像「没有耦合」。

### 落地物

- `tests/audit/host_beast_probe.cjs`（可复跑探针：成员切分 / 域账 / 外部引用 / 逐面自洽 / 拆分候选分诊）
- `tests/audit/host_beast_baseline.json`（**读数零手抄**：由脚本从探针输出直接落盘，避免转写漂移）
- `tests/v3232_host_beast_triage.test.mjs`（14 项：A 基线四件 / B 探针口径自证 / C 真跑读数一致
  （TOP40 按**同名第 k 条**对位）/ D 结论挂在读数上 / E 三条负控制（**镜像仓**里真跑探针）/ F 版本锚）

### 验证

定向 `v3232` **14/14**（含三条镜像仓负控制）；全量见下节门禁结果。

### 边界如实声明

① **未拆任何东西**：`index.js` 一行未动（16922 行保持不变），产品行为零变化；
② **未做 AST 级归属**：外部引用按名字统计，同名会混（本仓口径是文本形态，换工具应单独立一版）；
③ **未覆盖运行期可见性**：拆分的真正风险是「跨模块 this 丢失」，属实机面，无头门禁看不见；
④ **未排拆分优先级**：分诊表给三个读数，不排优先级（排优先级要一并看功能风险，那不在读数里）。

## v3.230.0

**主题：P-3 快照外供面规模取证 —— 把「快照要不要瘦」从印象变成读数，并按读数否掉一个候选。
本版只取证与立基线，未改任何契约、未做任何瘦身。**

### 为什么先量不先砍

`buildBridgeSnapshot()` 是本仓**唯一**的对外读数出口（13 个数据面 + meta），
但此前**没有任何规模读数**：「它随内容长多大、克隆要多久、成本花在哪一段」全凭印象。
而任何瘦身都动跨仓契约（下游 ruby-phone 12 个 App 在读它），
所以顺序只能是 **先量 → 再定形态**，反过来做就是拿猜测改契约。

### 实测读数（合成数据 + 真方法体，**非实机**）

| 档 | 负载字节 | 构造耗时 |
|---|---|---|
| 空宿主（13 面全部 present=false） | **117 B** | 0.005 ms |
| 100 角色 / 50 详情 / 50 场景 | 14 047 B | 0.602 ms |
| 400 / 50 / 50 | 35 917 B | 1.253 ms |
| 1600 / 50 / 50 | 124 597 B | 5.158 ms |
| 100 / 800 / 50 | 55 947 B | 0.955 ms |
| 100 / 50 / 800 | 53 698 B | 1.169 ms |

三条缩放曲线各自**近似线性**，未观察到超线性。

### 逐面字节账（N=400 档，负载 35 917 B）

`characters` **28 941 B = 80.6%** —— 其余 12 面合起来不到 20%
（lifeDetails 7.5% · scene 7.2% · moneyLedger 1.1% · evidence 0.8% · outline 0.7% ·
injection 0.6% · worldProg 0.3% · recallAudit 0.2% · protagonist 0.1% · worldLedgerRead 0.1% · clock 0.0%）。
恒定承载面（bridge/version/floor/exportedAt/pluginVersion + meta）合计 **1 192 B = 3.3%**。
一句话：**「快照大」在读数上等于「角色面大」**。

### 成本拆解（N=400 档同档对照）

深克隆段 **0.585 ms** / 自述段（JSON.stringify）**0.213 ms** / 端到端 **0.902 ms** ——
**65% 在深克隆段**。而成本**与字节成正比、与面数无关**（每面一个常数调用，127 B 与 28 941 B 走同一条路径）
⇒ 「砍面数」换不到耗时，「减字节」才换得到。

### 结论：按读数否掉「砍面数」这条路

① **没有一面是纯挂账** —— 13 面在手机端各有直接读者（`snap.<面>` 访问点实测：
scene 7 · lifeDetails 7 · worldProg 6 · protagonist 6 · outline 6 · worldLedgerRead 4 ·
moneyLedger 4 · clock 4 · characters 4 · recallAudit 1 · projection 1 · injection 1）；
② **砍读侧不给收益** —— 快照是**出口**不是缓存（每次构造都是新对象），
「不缓存」本身就是「读者永远拿到当下」的保证，去掉等于用陈旧读数换速度；
③ **恒定承载面已经很薄**（3.3%），砍掉换不回一个 percent。

**唯一有读数支持的瘦身轴不是「去面」而是「每面预算」**（为 `characters` 这类按规模放大的面设上限
+ **如实截断读数**）。这一轴本版**只取证、不动手** —— 形态决策不预支。

### 同轮口径留痕

- 探针自第一版即遵守本仓已立的两条口径纪律（**setup 不计时** / 每组独立实例，见 O-5 的 290 倍教训），
  本轮**未复现**那类错误，故没有「修前/修后差 N 倍」可记 —— 如实说明，不编一条不存在的修正。
- **`projection` 面读数 0 B 是口径结果、不是结论**：合成宿主没装 `_lastProjectionEnvelope`
  ⇒ present=false ⇒ 0 B。真实会话跑过管线时这一面会按 envelope 实际大小计入；
  基线里明写「**不得**把这里的 0 读成这一面没用」。
- 探针根目录由 `__dirname` 派生（不写死绝对路径）—— 本仓 `scan_cross_repo_binding` 的 P1 就拦绝对路径字面量。

### 落地物

- `tests/audit/snapshot_probe.cjs`（可复跑探针：四条缩放曲线 + 逐面字节账 + 两段成本拆解，
  输出自带 `synth: true` 与「setup 不计时」结构）
- `tests/audit/snapshot_baseline.json`（读数 / 缩放形状 / 逐面账 / 成本拆解 / 结论 / 口径修正 / **没做什么**）
- `tests/v3231_snapshot_scale_evidence.test.mjs`（12 项：A 基线四件 + 口径强制 + 缩放自洽 /
  B 探针口径（含**不得重演 O-5 口径错误**与位置无关性）/ C 真跑探针逐面账可复算 /
  D 结论必须挂在读数上且算术可复算 / E 三条负控制 / F 版本锚）

### 验证

定向 `v3230 + v3231` **21/21**；全量见下节门禁结果。

### 边界如实声明

① **未做任何瘦身、未改任何契约**（形态决策留给取证之后的版本）；
② 未验实机（无浏览器 / 无宿主 / 无真实聊天记录；合成数据只覆盖「面内条数」这一维，不覆盖真实结构长尾）；
③ **未覆盖快照的下游消费成本**（只量产出，没量手机端解析/渲染成本）；
④ 未覆盖严格 JSON 出口 `buildBridgeSnapshotJson()` 的字符串化总成本（本基线量的是 snapshot 那段）。
