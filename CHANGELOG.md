...
- **写入过两道门，任一不过即拒且账不动**：
  ① `idempotencyKey` 必填（缺 / 空白 ⇒ `missing-idempotency-key`）。内部 API 缺键时**本来就不幂等**
     （旧行为逐字保留），写入口必须比它严一档。
...
- 新增回归 23 项（`tests/v3214` 11 + `tests/v3215` 12），全部**先红后绿**；
  负控制一律真源码破坏 + 载入破坏副本 + 在副本上重跑同款判据（N1 链式拆门①、N2 丢掉「未放弃」限定、
  N3 回执漏键），工具自证锚点不存在 / 不唯一 / 同值替换必须抛。
- 全量门禁：**195/195 文件、1705 断言 0 失败、42/42 审计 `RC=0`**（测试段 42.7s、审计段 26.7s）。
