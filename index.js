                ...
                if (this.config.config.rubyPhoneBridge && extracted) {
                    try {
                        ...
                        if (bridge?.backfill) {
                            ...
                            const _backfillPayload = { ...extracted, clock: this.clock?.export?.() || null, pairs: this.pairMem?.export?.()?.pairs || [], lockedFacts: (this.config.config.lockedFactsEnabled !== false) ? this.summary.getLockedFacts() : [], deltaBook: this.deltaBook?.export?.() || null };
